...
class OclApp
{
  private:
    ...
    void execute_kernel(cl::Kernel kernel, size_t global_size,
                        size_t local_size, std::vector<cl::Event> &events)
    {
        cl::Event event;

        int err_num = queue_.enqueueNDRangeKernel(
            kernel, cl::NullRange, global_size, local_size, nullptr, &event);
  ...
  public:
    ...
    void bsort(std::vector<int> &vec, bool incr_order)
    {
        ...
        {
            for (uint passed_stage = 0; passed_stage < cur_stage + 1;
                 ++passed_stage)
            {
